<template>
  ...
  tf.setBackend('webgl');  // Используем WebGL для ускорения на GPU в браузере
}
    ...
    if (canvas.value && video.value) {
      ...
      if (ctx) {
        ...
        predictions.forEach((prediction) => {
          ...
          ctx.beginPath();
          ctx.rect(x, y, width, height); // Рисуем прямоугольник вокруг объекта
          ctx.lineWidth = 2;
          ctx.strokeStyle = 'red';
          ctx.fillStyle = 'red';
          ctx.stroke();
          ctx.fillText(
            `${prediction.class} (${Math.round(prediction.score * 100)}%)`, // Текст с именем и вероятностью
            x,
            y > 10 ? y - 5 : 10
          );
        });
